"use client";
import Modal from "@components/ui/modal";
import { FC, useState } from "react";
import { Priority, Status, Task, TaskData, User } from "@api/models";
import { DeleteTask, EditTask } from "@services/projectsService";


interface TaskEditFormProps {
    onTaskEdited: () => void;
    users?: User[];
    priorities?: Priority[] | null;
    statuses?: Status[] | null;
    task?: Task | null;
    isModalOpen: boolean;
    closeModal: () => void;
    role: string;
}

const TaskEditForm: FC<TaskEditFormProps> = ({ onTaskEdited: onTaskAdded, users, priorities, statuses, task, isModalOpen, closeModal, role}) => {
    const [isLoading, setIsLoading] = useState(false);
    const [deleteLoading, setDeleteLoading] = useState(false);

    const handleSubmit = async (event: React.FormEvent) => {
        event.preventDefault();
        setIsLoading(true);
        try {
            const formData = new FormData(event.target as HTMLFormElement);
            const taskData: TaskData = {
                name: formData.get('name') as string,
                priorityId: parseInt(formData.get('priorityId') as string),
                statusId: parseInt(formData.get('statusId') as string),
                createdDate: task?.createdDate as string,
                deadline: role === "Employee"? task?.deadline as string : formData.get('deadline') as string,
                executorId: role === "Employee"? task?.executorId as string : formData.get('executorId') as string,
                projectId: task?.projectId as number,
                description: formData.get('description') as string
            };

            if (!task?.id) {
                throw new Error('Task ID is undefined');
            }

            await EditTask(task.id, taskData);
            closeModal();
            onTaskAdded();
        } catch (error) {
            alert('Failed to edit task. Please try again.');
            console.error('Failed to edit task:', error);
        } finally {
            setIsLoading(false);
        }
    };

    const hadleTaskDelete = async () => {
        if (!task?.id) {
            throw new Error('Task ID is undefined');
        }
        setDeleteLoading(true);
        try {
            const confirmDelete = confirm('Are you sure you want to delete this task?');
            if (!confirmDelete) {
                return;
            }
            await DeleteTask(task.id);
            closeModal();
            onTaskAdded();
        } catch (error) {
            alert('Failed to delete task. Please try again.');
            console.error('Failed to delete task:', error);
        } finally {
            setDeleteLoading(false);
        }
    };

    return (
        <div className="">
            <Modal isOpen={isModalOpen} onClose={closeModal} title="Edit Task">
                <form onSubmit={handleSubmit} className="p-4 md:p-5">
                    <div className="grid gap-4 mb-4 grid-cols-2">
                        <div className="col-span-2">
                            <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                            <input
                                type="text"
                                name="name"
                                id="name"
                                defaultValue={task?.name}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-600 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Type task name"
                                required
                            />
                        </div>
                        <div className="col-span-2">
                            <label htmlFor="description" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
                            <textarea
                                name="description"
                                id="description"
                                defaultValue={task?.desciption}
                                rows={4}
                                className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Write task description here"
                            ></textarea>
                        </div>
                        <div className="">
                            <label htmlFor="statusId" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Statuses
                            </label>
                            <select
                                name="statusId"
                                id="statusId"
                                defaultValue={task?.statusId}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-600 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                required
                            >
                                {statuses?.map(statuse => (
                                    <option key={statuse.id} value={statuse.id}>
                                        {statuse.name}
                                    </option>
                                ))}
                            </select>
                        </div>
                        <div className="">
                            <label htmlFor="priorityId" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Priority
                            </label>
                            <select
                                name="priorityId"
                                id="priorityId"
                                defaultValue={task?.priorityId}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-600 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                required
                            >
                                {priorities?.map(priority => (
                                    <option key={priority.id} value={priority.id}>
                                        {priority.name}
                                    </option>
                                ))}
                            </select>
                        </div>
                        <div className={"col-span-2 " + (role === 'Employee' ? 'hidden' : '')}>
                            <label htmlFor="executorId" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Executor
                            </label>
                            
                            <select
                                name="executorId"
                                id="executorId"
                                defaultValue={task?.executorId}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-600 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                required
                                disabled={role === 'Employee'}
                            >
                                {users?.map(user => (
                                    <option key={user.id} value={user.id}>
                                        {user.firstName} {user.lastName}
                                    </option>
                                ))}
                            </select>
                        </div>
                        <div className={"col-span-2 "+ (role === 'Employee' ? 'hidden' : '')}>
                            <label htmlFor="deadline" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Deadline</label>
                            <input
                                type="date"
                                name="deadline"
                                id="deadline"
                                defaultValue={task?.deadline.split('T')[0]}
                                className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-600 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                required
                                disabled={role === 'Employee'}
                            />
                        </div>
                    </div>
                    <div className="flex justify-between w-full">
                        <button
                            type="button"
                            className={"btnSecondary w-1/3 " + (role === 'Employee' ? 'hidden' : '')}
                            disabled={deleteLoading || role === 'Employee'}
                            onClick={hadleTaskDelete}
                        >
                            {deleteLoading && (
                                <svg aria-hidden="true" role="status" className="inline w-4 h-4 me-3 text-gray-200 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="#1C64F2" />
                                </svg>
                            )}
                            {deleteLoading ? 'loading' : 'Delete'}
                        </button>
                        <button
                            type="submit"
                            className="btnPrimary w-1/2"
                            disabled={isLoading}
                        >
                            {isLoading && (
                                <svg aria-hidden="true" role="status" className="inline w-4 h-4 me-3 text-gray-200 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="#1C64F2" />
                                </svg>
                            )}
                            {isLoading ? 'loading' : 'Save changes'}
                        </button>
                    </div>


                </form>
            </Modal>
        </div>
    );
}

export default TaskEditForm;